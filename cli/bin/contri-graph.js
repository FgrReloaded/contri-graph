...
function buildManagedBlock(shell) {
    const command = 'if command -v contri-graph >/dev/null 2>&1; then contri-graph "$CONTRI_GRAPH_USER"; else npx -y contri-graph "$CONTRI_GRAPH_USER"; fi';
    if (shell === "fish") {
        return `${BLOCK_START}
...
async function installShellHook(targetShell) {
