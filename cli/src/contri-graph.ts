...
function buildManagedBlock(shell: ShellName, username: string): string {
...
async function resolveInstallUsername(inputUsername: string): Promise<string> {
  if (inputUsername.trim()) return inputUsername.trim();

  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    console.error(
      "No username provided. Pass one with --user <github-username> when running non-interactively.",
    );
    process.exit(1);
  }
...
async function installShellHook(targetShell: string, inputUsername: string): Promise<void> {
